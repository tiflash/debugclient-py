...
def get_cfg_option(section, option):
    ...
    if not os.path.isfile(env_cfg_path):
        raise Exception(
            "Could not find environment configuration file: %s " % env_cfg_path
        )
...
def configure_cc13x0(cc13x0=None):
    ...
    cc13x0_ccxml = os.path.join(
        get_repo_path(), "tests", "resources", "cc13x0", "cc13x0.ccxml"
    )
...
def main():
    ...
    parser.add_argument(
        "--ccs",
        help="full path to ccs exe; overrides the value set in tests/%s"
        % DEFAULT_ENV_CFG,
        type=str,
    )
    parser.add_argument(
        "--cc13x0",
        help="serial number for cc13x0 device; overrides the value set in tests/%s"
        % DEFAULT_ENV_CFG,
        type=str,
    )
